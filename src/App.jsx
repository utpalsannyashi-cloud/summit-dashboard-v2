...
export default function App() {
  ...
  const saveOfficer = async ()=>{
    ...
    const durationStr = (oForm.deployment_start && oForm.deployment_end) ? `${oForm.deployment_start} to ${oForm.deployment_end}` : (oForm.deployment_start || oForm.deployment_end || '');
    const payload = { name: oForm.name, designation: oForm.designation, contact: oForm.contact, current_vertical: oForm.current_vertical, origin_station: oForm.origin_station, deployment_duration: durationStr };
    if(modalData.id) await supabase.from('sd_officers').update(payload).eq('id',modalData.id).eq('team_id',teamId);
    else await supabase.from('sd_officers').insert({id:crypto.randomUUID(), ...payload, team_id:teamId, created_at:new Date().toISOString()});
    setModal(null);
  };
...
Be concise and professional.`;
  ...
  {!tForm.is_parallel&&<div>
    <label style={{fontSize:11,color:t.muted,display:'block',marginBottom:4}}>Chain After Task</label>
    <select value={tForm.parent_task_id} onChange={e=>setTForm(f=>({...f,parent_task_id:e.target.value}))} style={inp}>
      <option value=''>Select task to chain after...</option>
      {Object.values(tasks).filter(tk=>tk.vertical_id===tForm.vertical_id&&tk.id!==modalData.id).map(tk=><option key={tk.id} value={tk.id}>{tk.title}</option>)}
    </select>
  </div>}
</div>
